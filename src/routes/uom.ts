...
SPUOMRouter.post("/addUOM", async (req, res) => {
    ...
    try {
        const checkDuplicate = await UOMManager.findOne(SP_UomTypes, { uom_type : uom_name })
        ...
        if (checkDuplicate !== undefined) {
            ...
            return res.send({ message : `Redundant on UOM ${ uom_name }.`, status : false })
        }
...
SPUOMRouter.post("/editUOM", async (req, res) => {
    ...
    try {
        await UOMManager.update(SP_UomTypes, { id },
            {
                uom_type
            }
        )
